...
export function formatProductResponse(products, searchTerms) {
  ...
  if (products.length === 0) {
    ...
    // Simply mention that products were found without listing them in the chat message
    response = `I found ${products.length} products matching "${searchTerms}". Check out the product section below.`;
  }
  return response;
}
