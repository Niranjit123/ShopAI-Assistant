...
export const faqAgent = {
  handle: async (query, entities = {}, conversationHistory = []) => {
    try {
      ...
      const topContextItems = retrievedFaqs.slice(0, 2); // Take top 2 results
      const ragContext = topContextItems.map(item => `Question: ${item.question}\nAnswer: ${item.text_chunk}`).join('\n\n'); // More structured context

      const prompt = `
        Based on the following information, answer the user's question concisely.
        If the information doesn't directly answer, say you couldn't find a specific answer.
        ...
        Answer:
      `;
      const agentResponse = await gemini.generateResponse(prompt, currentMessageHistory, 500); // Example: max 500 output tokens
